...
import { getLyricAdapter } from '@infra/pluginManager/main/getLyricAdapter';
...
class DownloadManager {
            ...
            await dt.execute(
                ...
                () => {
                    ...
                    try {
                        ...
                        // 嵌入封面和歌词到音频文件
                        this.embedMetadata(task, musicItem).catch((err) => {
                            console.warn('[DownloadManager] Failed to embed metadata:', err);
                        });
    ...
    private async embedMetadata(
        ...
        try {
            ...
            const metadata: any = {};
            console.log('[DownloadManager] Metadata to musicItem:', musicItem);

            // 获取并嵌入封面
            if (musicItem.artwork) {
                try {
                    const picUrl = await this.getPicUrl({ musicInfo: musicItem });
                    if (picUrl) {
                        const artworkResponse = await axios.get(picUrl, {
                        ...
                        if (artworkResponse.data) {
                            const imageBuffer = Buffer.from(artworkResponse.data);
                            const contentType =
                                (artworkResponse.headers['content-type'] as string) ||
                                (picUrl.endsWith('.png') ? 'image/png' : 'image/jpeg');

                            const success = await this.writePicToFile(
                                task.filePath,
                                imageBuffer,
                                contentType,
                            );
                            ...
                            if (success) {
                                console.log(
                                    '[DownloadManager] Embedded cover artwork to',
                                    task.filePath,
                                );
                            } else {
                                console.warn(
                                    '[DownloadManager] WritePicToFile returned false for',
                                    task.filePath,
                                );
                            }
                        } else {
                            console.warn('[DownloadManager] Artwork response has no data:', picUrl);
                        }
                    }
                } catch (err) {
                    console.warn('[DownloadManager] Failed to download artwork:', err);
                }
            }

            // 获取并嵌入歌词
            try {
                const lyricSource = await getLyricAdapter(
                    { musicItem },
                    (params) =>
                        (this.pluginManager as any).callPluginMethod?.(params) ||
                        Promise.resolve(null),
                    this.mediaMeta,
                    this.musicItemProvider,
                );
                console.log('[DownloadManager] Lyric source:', lyricSource);
                if (lyricSource?.rawLrc) {
                    metadata.unsynchronisedLyrics = {
                        language: 'eng',
                        text: lyricSource.rawLrc,
                    };
                }
            } catch (err) {
                console.warn('[DownloadManager] Failed to get lyrics:', err);
            }

            // 只有当有元数据要嵌入时才进行写入
            if (Object.keys(metadata).length > 0) {
                const success = await this.writeMetadataToFile(task.filePath, metadata);
                if (success) {
                    console.log(`[DownloadManager] Embedded metadata to ${task.filePath}`);
                }
            }
        } catch (err) {
            console.warn('[DownloadManager] Failed to embed metadata:', err);
    ...
    private async getPicUrl({
        ...
        musicInfo: IMusic.IMusicItem;
    }): Promise<string | null> {
        // 直接返回musicItem中的artwork URL
        // 在未来可以扩展为从多个来源获取封面或进行URL处理
        return musicInfo.artwork || null;
    }

    /** 嵌入封面到音频文件，目前仅支持 MP3 */
    private async writePicToFile(
        filePath: string,
        imageData: Buffer,
        ...
        if (ext !== '.mp3') {
            console.warn('[DownloadManager] writePicToFile only supports MP3 for now:', filePath);
            return false;
        }
        ...
        try {
            ...
            // 优先 update，避免覆盖已有其它标签；fallback write
            let success = nodeID3.update(tags, filePath);
            if (!success) {
                success = nodeID3.write(tags, filePath);
            }

            if (!success) {
                console.warn('[DownloadManager] node-id3 failed to set cover image:', filePath);
            }

            return !!success;
        } catch (err) {
            console.warn('[DownloadManager] Failed to write pic to file:', err);
            return false;
        }
    }

    /** 写入元数据到音频文件 */
    private async writeMetadataToFile(filePath: string, metadata: any): Promise<boolean> {
        return new Promise((resolve) => {
            const ext = path.extname(filePath).toLowerCase();
            try {
                if (ext === '.mp3') {
                    // 对于MP3文件使用node-id3，使用 update 避免覆盖已有图片/标签
                    const nodeID3 = require('node-id3') as typeof import('node-id3');
                    const success = nodeID3.update(metadata, filePath);
                    resolve(!!success);
                } else if (['.m4a', '.m4b'].includes(ext)) {
                    // 对于M4A文件，目前先跳过（需要其他库支持）
                    console.log(
                        `[DownloadManager] M4A metadata embedding not yet supported for ${filePath}`,
                    );
                    resolve(false);
                } else {
                    resolve(false);
                ...
                console.warn('[DownloadManager] Error writing metadata:', err);
                resolve(false);
            }
        });
    }
    ...
    private buildFileName(musicItem: IMusic.IMusicItem): string {
        ...
        const format = this.appConfig.getConfigByKey('download.filenameFormat') ?? 'title-artist';

        const fileName =
            format === 'title-artist' ? `${title} - ${artist}` : `${artist} - ${title}`;
        return sanitizeFileName(fileName, 170);
    }
